    ...
    while True:
        ...
        if not rows:
            ...
            header = font_small.render(
                f"{'#':<4} {'Name':<16} {'Score':<8} {'Lv':<5} {'Date'}", True, colorGRAY
            )
            ...
            for rank, username, score, level, date in rows:
                ...
                line  = font_small.render(
                    f"{rank:<4} {username:<16} {score:<8} {level:<5} {date}", True, color
                )
...
def play_game(username):
    ...
    POWERUP_SPAWN_INTERVAL = 10_000   # try to spawn every 10 s
    ...
    while True:
        ...
        # BUG FIX: was `current_time = now` (local variable, never used).
        # Now correctly resets food.cooldown_start via generate_random_pos.
        if now - food.cooldown_start > COOLDOWN_TIME:
            food.generate_random_pos(snake.body, obstacles)

        # Power-up pickup / expiry
        if powerup:
            head = snake.body[0]
